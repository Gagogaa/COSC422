...
; uncomment following two lines if using 16f627 or 16f628.
    ...
    cblock  0x20    ;start of general purpose registers
        ...
        straightOrTurn ;used to determin if the robot should turn or go forward
    endc
...
B0interrupt ;check the number in TMR0 and set led
    ...
    bsf     straightOrTurn, 1   ;set to one so the robot turns 
    btfsc   STATUS, C
    bcf     straightOrTurn, 1   ;set to zero so the robot goes straight
...
T0interrupt     ;Repulse the sensor when TMR0 overflows
    ...
    ;; if straightorturn is set to 1 turn else go forward
    btfss   straightOrTurn, 1
    call    goForward
    btfsc   straightOrTurn, 1
    call    turn
